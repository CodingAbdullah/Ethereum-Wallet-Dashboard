...
exports.handler = async (event) => { // Changed to AWS Lambda handler
    ...
    try {
        const response = await axios.get('https://deep-index.moralis.io/api/v2/nft/' + address + "/" + id + LOOKUP_ENDPOINT + "?chain=" + network + "&format=decimal", options); // Use await for axios call
