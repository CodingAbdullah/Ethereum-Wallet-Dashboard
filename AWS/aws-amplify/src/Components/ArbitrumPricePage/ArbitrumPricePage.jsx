...
const ArbitrumPricePage = () => {
    ...
    const QUERY_STRING = '?ids=arbitrum' + '&vs_currencies=usd&include_24hr_change=true';
    const CURRENCY_ENDPOINT = '/simple/price';
    const QUERY_STRING_PRICES = "?vs_currency=usd&days=14"; // Default selection for now.
    const PRICE_ENDPOINT = "/coins/arbitrum/market_chart" + QUERY_STRING_PRICES + "&interval=daily";
  ...
  useEffect(() => {
    const fetchCoins = async (value) => {      
      // Update Naming
      if (value === '14'){
        await fetch(URL + "/coins/arbitrum/market_chart" + QUERY_STRING_PRICES + "&interval=daily")
        .then(response => response.json())
        .then(res => {
          setChartData(prevState => {
            let days = [];
        ...
        await fetch(URL + "/coins/arbitrum/market_chart?vs_currency=usd&days=30&interval=daily") // Generic id setup
        .then(response => response.json())
        .then(res => {
          setChartData(prevState => {
            let days = [];
        ...
        await fetch(URL + "/coins/arbitrum/market_chart?vs_currency=usd&days=1&interval=hourly") // Generic id setup
        .then(response => response.json())
        .then(res => {
          setChartData(prevState => {
            let days = [];
       ...
       // Current coin price 
       await fetch(URL + CURRENCY_ENDPOINT + '?ids=arbitrum&vs_currencies=usd&include_24hr_change=true') // Get current coin price
       .then(response => response.json())
       .then(res => {
         if (res[Object.keys(res)[0]] !== undefined) { // Get coin value from generic setup res.ethereum, res.binance ... and so on
           updateCoinInfo((prevState) => {
