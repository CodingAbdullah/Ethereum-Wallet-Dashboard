...
exports.ensTransfersByName = (req, res) => {
    ...
    sdk.get('/ens-transfers-by-name?ens_name=' + ensName + '&transfer_category=all&order=asc', { 'x-api-key': process.env.TRANSPOSE_API_KEY })
    .then(response => {
        res.status(200).json({
...
exports.ensTransfersById = (req, res) => {
    ...
    sdk.get('/ens-transfers-by-token-id?token_id=' + id + '&transfer_category=all&order=asc', { 'x-api-key': process.env.TRANSPOSE_API_KEY })
    .then(response => {
        res.status(200).json({
            information: response // Send response with status 200 and information
        })
